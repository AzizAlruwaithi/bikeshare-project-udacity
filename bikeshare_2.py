...
def get_filters():
    ...
    if date_filter.lower() not in both_s:

        if date_filter.lower() in day_q:
        ...
        if date_filter.lower() in month_q:
...
def trip_duration_stats(df):
    ...
    start_time = time.time()
    df_df = pd.DataFrame(df[['Trip Duration']])
    # display total travel time
    total_traveled_time = pd.DataFrame(df_df.sum())
    print('The Total Traveled time is:', ((str(total_traveled_time.values/(60*60))).replace('[[','')).replace(']]',''),
          'Hours')
    # display mean travel time
    avg_traveled_time = pd.DataFrame(df_df.mean())
    print('The Average Traveled Time:', ((str(avg_traveled_time.values/(60))).replace('[[','')).replace(']]',''),
          'Minutes')
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
...
def user_stats(df, city):
    ...
    question = input('Would like to see 5 lines of row data? \n (yes/no)')
    df_df = pd.DataFrame(df)
    while question.lower() == 'yes':
        r_num = np.random.randint(0,df_df.index.max())
        print(pd.DataFrame(df_df[r_num: r_num+5]))
        question = input('Would like to see 5 lines of row data? \n (yes/no)')
